...
def load_baseline_weights_to_se_model(se_model, baseline_checkpoint_path):
    """
    将基线模型的权重加载到SE模型中
    ...
    # 获取SE模型的状态字典
    se_state_dict = se_model.state_dict()
    
    # 映射基线模型的权重到SE模型
    # 基线模型结构: backbone.X -> SE模型结构: conv1, bn1, layer1, layer2, layer3, layer4
    weight_mapping = {}
    ...
    # 加载权重到SE模型（忽略SE相关的参数和分类器）
    missing_keys = []
    loaded_keys = []
    
    for key in weight_mapping:
        if key in se_state_dict:
            # 检查形状是否匹配
            if weight_mapping[key].shape == se_state_dict[key].shape:
                se_state_dict[key] = weight_mapping[key]
                loaded_keys.append(key)
            else:
                missing_keys.append(f"{key} (shape mismatch: {weight_mapping[key].shape} vs {se_state_dict[key].shape})")
        else:
            missing_keys.append(key)
    
    # 加载权重
    se_model.load_state_dict(se_state_dict, strict=False)
    
    print(f"成功加载基线模型权重")
    print(f"已加载的权重: {len(loaded_keys)} 个")
    if missing_keys:
        print(f"跳过的权重: {len(missing_keys)} 个")
    print("注意: 分类器权重将从头开始训练")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='Train SE-Net attention model')
    
    # SE-Net特定参数
    parser.add_argument('--reduction', type=int, default=16, help='SE reduction ratio')
    ...
    parser.add_argument('--experiment_name', type=str, default='resnet50_se_net')
    ...
    print("="*60)
    print("🔥 SE-Net 注意力机制训练开始")
    print("="*60)
    print(f"实验配置:")
    print(f"  - 注意力类型: SE-Net (Squeeze-and-Excitation)")
    ...
    if args.load_baseline:
        ...
        model = load_baseline_weights_to_se_model(model, args.load_baseline)
    else:
    ...
    print(f"  - SE模块数量: {model_info['se_blocks']}")
